    ...
    def _validate_file(self):
        ...
        with open(self.filename, 'r', newline=None) as data_file:
            ...
            try:
                ...
                if '\t' not in sample_content:
                    self.logger.warning('Could not detect a tab separator. Confirm that all separators are tabs.')
                    return
                else:
                    self.logger.warning('Could not reliably detect file structure. Continuing validation to check for inconsistent column counts.')
...
class WsiValidator(Validator):
    ...
    def _stain_contract_error(row):
        ...
        if is_hne and is_ihc:
            return 'WSI row cannot be both H&E and IHC'
        expected = 'H&E' if is_hne else 'IHC' if is_ihc else 'Other'
        if row.get('SLIDE_TYPE') != expected:
            return 'SLIDE_TYPE is inconsistent with IS_HNE and IS_IHC'
        return None
    ...
    def _validate_file(self):
        ...
        for line_number, line in enumerate(lines[5:], start=6):
            ...
            stain_error = self._stain_contract_error(row)
            if stain_error:
                self._error(stain_error, line_number,
                            self.EXPECTED_HEADERS.index('SLIDE_TYPE'), row['SLIDE_TYPE'])
